...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    if (
      variant === 'secondary' && // if the button is a secondary variant
      colorScheme !== 'black' && // and the button is not black
      colorScheme !== 'white' // and the button is not white
    ) {
      console.log('Not allowed', variant, colorScheme);
      throw new Error(
        `The secondary variant is only available for the black and white colorSchemes.`,
      );
    } else if (variant === 'elevated' && colorScheme === 'white') {
      console.log('Not allowed', variant, colorScheme);
      throw new Error(
        `The elevated variant is not available for the white colorScheme.`,
      );
    } else if (colorScheme === 'gold-gradient' && variant !== 'primary') {
      console.log('Not allowed', variant, colorScheme);
      throw new Error(
        `The gold-gradient colorScheme is only available in the primary variant.`,
      );
    }
    ...
    return (
      <button
        ...
        type={type}
        className={cn(
          'group/button relative flex shrink-0 items-center justify-center overflow-hidden rounded-md transition',
